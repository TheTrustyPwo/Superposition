...
class NSlitSimulation extends Simulation {
   ...
   drawScreenView = (screenCtx, width, height) => {
      // defensive: if no screen or context, do nothing
      if (!screenCtx || !this.screen) return;

      screenCtx.clearRect(0, 0, width, height);

      // compute left/right edges of the screen area
      const leftEdgeX  = this.screen.x - this.screen.w / 2;
      const rightEdgeX = this.screen.x + this.screen.w / 2;

      // compute angular range from grating to screen edges using physical distance
      const minAngle = Math.atan(((leftEdgeX - this.slitX) * this.xpx2m) / Math.max(1e-12, this.distanceToScreen));
      const maxAngle = Math.atan(((rightEdgeX - this.slitX) * this.xpx2m) / Math.max(1e-12, this.distanceToScreen));

      for (let px = 0; px < width; px++) {
          const ratio = px / Math.max(1, width - 1);
          const theta = minAngle + ratio * (maxAngle - minAngle);

          const intensity = this.evaluate(theta); // normalized 0..1
          const color = interpolate(0, this.color, intensity);

          screenCtx.fillStyle = color;
          screenCtx.fillRect(px, 0, 1, height);
      }
  };
